...
class _DummyPopen:
    ...
    def wait(self, _timeout: int | None = None) -> int:
        return 0
...
def _push_log(
    ...
    *,
    ...
    with collector._lock:
        collector._history.append(entry)
    collector._queue.put((buffer_name, line))
...
def test_take_snapshot_full_profile_writes_meminfo_and_dmesg(tmp_path: Path) -> None:
    ...
    def fake_run(cmd: list[str], **_kwargs) -> _DummyCompletedProcess:
...
def test_take_snapshot_full_profile_runs_shell_dumps_concurrently(
    ...
    collector = AdbLogCollector()
    _push_log(collector, "main", "window", timestamp=10.0)
...
def test_take_snapshot_light_profile_keeps_telephony_and_logcat(
    ...
    collector = AdbLogCollector()
    _push_log(collector, "main", "before-window", timestamp=10.0)
    _push_log(collector, "radio", "radio-window", timestamp=20.0)
    _push_log(collector, "main", "main-window", timestamp=25.0)
    _push_log(collector, "main", "after-window", timestamp=40.0)
...
def test_take_snapshot_uses_collector_window_for_logcat(tmp_path: Path) -> None:
    collector = AdbLogCollector()
    _push_log(collector, "main", "before-window", timestamp=10.0)
    _push_log(collector, "radio", "radio-first", timestamp=20.0)
    _push_log(collector, "main", "main-second", timestamp=25.0)
    _push_log(collector, "crash", "boundary-window", timestamp=30.0)
    connector = AdbConnector(serial="SER123")
...
class AdbLogCollectorTests(unittest.TestCase):
    ...
    def test_slice_uses_non_overlapping_time_windows(self) -> None:
        collector = AdbLogCollector()
        _push_log(collector, "main", "first", timestamp=10.0)
        _push_log(collector, "main", "boundary", timestamp=20.0)
        _push_log(collector, "radio", "later", timestamp=30.0)
...
class AdbAnomalyDetectorTests(unittest.TestCase):
    ...
    def test_drain_events_clears_buffer(self) -> None:
        ...
        self.assertEqual(detector.drain_events(), [])
