...
class SIPSenderReactorTests(unittest.TestCase):
    ...
    def test_target_endpoint_rejects_ipsec_alias(self) -> None:
        with self.assertRaises(ValidationError):
            TargetEndpoint.model_validate(
                {
                    "mode": "real-ue-direct",
                    "msisdn": "111111",
                    "ipsec_mode": "ipsec",
    ...
    def test_send_real_ue_direct_container_path_reports_legacy_event(self) -> None:
            ...
            patch(
                ...
                return_value=type(
                    ...
                    (),
                    {
                        "host": "10.20.20.8",
                        "port": 5060,
                        "label": "static:10.20.20.8:5060",
                        "observer_events": ("resolver:test:10.20.20.8:5060",),
                    },
                )(),
            ),
            patch(
                "volte_mutation_fuzzer.sender.core.prepare_real_ue_direct_payload",
                return_value=(b"normalized-payload", ("direct-normalization:test",)),
            ),
            patch(
                "volte_mutation_fuzzer.sender.container_exec.send_via_container",
                return_value=type(
                    "ContainerResult",
                    (),
                    {
                        ...
                        "observer_events": ("container-driver:ok",),
                    },
                )(),
            ) as mock_container_send,
        ):
            result = self.reactor.send_artifact(artifact, target)

        self.assertEqual(result.outcome, "success")
        self.assertIn("container-send:legacy:legacy-netns", result.observer_events)
        self.assertNotIn(
            "container-send:deprecated:legacy-netns", result.observer_events
        )
        mock_container_send.assert_called_once()
