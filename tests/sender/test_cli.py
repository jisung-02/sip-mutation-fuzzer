...
class SIPSenderCLITests(unittest.TestCase):
    ...
    def test_send_response_command_rejects_ipsec_alias(self) -> None:
        result = self.runner.invoke(
            app,
            [
                "send",
        ...
        self.assertIn("Invalid value for --ipsec-mode", result.output)
        self.assertIn("null,bypass,native", result.output)
        self.assertNotIn("validation error for TargetEndpoint", result.output)
