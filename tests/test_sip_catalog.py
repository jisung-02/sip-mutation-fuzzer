...
from volte_mutation_fuzzer.sip.common import AbsoluteURI, AuthChallenge, CSeqHeader, NameAddress, SIPURI, ViaHeader
...
class SIPCatalogTests(unittest.TestCase):
    ...
    def test_invite_requires_contact(self) -> None:
        ...
        with self.assertRaises(Exception):
            invite_model(
                ...
                max_forwards=70,
                from_=NameAddress(uri=SIPURI(scheme="sip", user="caller", host="example.com"), parameters={"tag": "a"}),
                to=NameAddress(uri=SIPURI(scheme="sip", user="callee", host="example.com")),
                call_id="call-1",
    ...
    def test_message_body_is_optional_but_contact_is_forbidden(self) -> None:
        ...
        packet = message_model(
            request_uri=SIPURI(scheme="sip", host="ue.example.com"),
            via=[ViaHeader(host="proxy.example.com", branch="z9hG4bK-2")],
            max_forwards=70,
            from_=NameAddress(uri=SIPURI(scheme="sip", user="sender", host="example.com"), parameters={"tag": "a"}),
            to=NameAddress(uri=SIPURI(scheme="sip", user="ue", host="example.com")),
            call_id="call-2",
        ...
        with self.assertRaises(Exception):
            message_model(
                ...
                call_id="call-2",
                cseq=CSeqHeader(sequence=2, method=SIPMethod.MESSAGE),
                contact=[NameAddress(uri=SIPURI(scheme="sip", user="sender", host="example.com"))],
            )
    ...
    def test_401_requires_www_authenticate(self) -> None:
        ...
        with self.assertRaises(Exception):
            unauthorized_model(
                ...
                from_=NameAddress(uri=SIPURI(scheme="sip", user="registrar", host="example.com"), parameters={"tag": "reg"}),
                to=NameAddress(uri=SIPURI(scheme="sip", user="ue", host="example.com"), parameters={"tag": "ue"}),
                call_id="call-3",
                cseq=CSeqHeader(sequence=3, method=SIPMethod.REGISTER),
            )

        packet = unauthorized_model(
            via=[ViaHeader(host="proxy.example.com", branch="z9hG4bK-3")],
            from_=NameAddress(uri=SIPURI(scheme="sip", user="registrar", host="example.com"), parameters={"tag": "reg"}),
            to=NameAddress(uri=SIPURI(scheme="sip", user="ue", host="example.com"), parameters={"tag": "ue"}),
            call_id="call-3",
            cseq=CSeqHeader(sequence=3, method=SIPMethod.REGISTER),
            www_authenticate=(AuthChallenge(realm="example.com", nonce="nonce-1"),),
        )
    ...
    def test_469_requires_recv_info(self) -> None:
        ...
        with self.assertRaises(Exception):
            bad_info_model(
                ...
                from_=NameAddress(uri=SIPURI(scheme="sip", user="remote", host="example.com"), parameters={"tag": "remote"}),
                to=NameAddress(uri=SIPURI(scheme="sip", user="ue", host="example.com"), parameters={"tag": "ue"}),
                call_id="call-4",
    ...
    def test_via_ttl_zero_and_cseq_upper_bound(self) -> None:
        via = ViaHeader(host="proxy.example.com", branch="z9hG4bK-ttl", ttl=0, transport="TLS-SCTP")
