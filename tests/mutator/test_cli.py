...
class SIPMutatorCLITests(unittest.TestCase):
    ...
    def test_packet_command_warns_with_mutations_per_case_for_targeted_multi_mutation(
        self,
    ) -> None:
        baseline_json = self.generate_request_baseline_json("OPTIONS")
        ...
        self.assertEqual(payload["final_layer"], "wire")
        self.assertIn("warning: --mutations-per-case=3 ignored", result.output)
        self.assertNotIn("--max-operations=3 ignored", result.output)

    def test_packet_command_rejects_max_operations_option(self) -> None:
        baseline_json = self.generate_request_baseline_json("OPTIONS")
    ...
    def test_help_exposes_basic_mutation_options(self) -> None:
        ...
        for command in ("packet", "request", "response"):
            with self.subTest(command=command):
                ...
                self.assertIn("--mutations-per-case", result.output)
                self.assertNotIn("--max-operations", result.output)
