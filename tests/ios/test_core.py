...
def _push_syslog(collector: IosSyslogCollector, line: IosSyslogLine) -> None:
    with collector._lock:
        collector._lines.append(line)
...
def test_take_snapshot_writes_syslog(tmp_path: Path) -> None:
    collector = IosSyslogCollector()
    _push_syslog(
        collector,
        IosSyslogLine(
...
class IosSyslogCollectorHealthTests(unittest.TestCase):
    def test_slice_filters_by_host_ts(self) -> None:
        collector = IosSyslogCollector()
        _push_syslog(collector, IosSyslogLine(host_ts=10.0, line="a"))
        _push_syslog(collector, IosSyslogLine(host_ts=20.0, line="b"))
        _push_syslog(collector, IosSyslogLine(host_ts=30.0, line="c"))
    ...
    def test_start_captures_all_processes_no_filter(self) -> None:
        collector = IosSyslogCollector(IosCollectorConfig(udid="ABC-123"))
...
class IosAnomalyDetectorTests(unittest.TestCase):
    ...
    def test_max_events_enforced(self) -> None:
        ...
        self.assertEqual(len(detector.drain_events()), 2)
