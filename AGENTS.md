...
- `campaign run` CLI 의 real-UE 기본값은 `--target-msisdn 111111` 및 `--ipsec-mode native` 이다. 다른 UE나 null/bypass 실험은 명시적으로 옵션을 덮어쓴다.
...
# 설정 불필요. 기본 target MSISDN은 111111이며, live resolver 가 UE 를 못 찾는 환경(예: kamctl Contact 가 user-less
...
# After (간단)
--target-msisdn <TARGET_MSISDN> --ipsec-mode native
```
