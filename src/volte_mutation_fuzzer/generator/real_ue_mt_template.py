...
The bundled template files under ``generator/templates/`` contain 3GPP-compliant
MT INVITE shapes, with {{slot}} placeholders for fields that vary per-call,
per-session, or per-network environment.
...
Usage::
    ...
    template_text = load_mt_invite_template()
...
def load_mt_invite_template() -> str:
    """Return the raw default 3GPP MT INVITE template text.

    The returned text uses ``\\n`` line endings; ``render_mt_invite`` normalises
    to CRLF.
    """
    resource_name = "mt_invite_3gpp.sip.tmpl"
