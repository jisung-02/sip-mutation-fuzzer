...
_IPSEC_MODES: tuple[str, ...] = ("null", "bypass", "native")
...
def _build_target(
    *,
    ...
    if ipsec_mode is not None and ipsec_mode not in _IPSEC_MODES:
        raise typer.BadParameter(
            f"--ipsec-mode must be one of {','.join(_IPSEC_MODES)}",
            param_hint="--ipsec-mode",
        )
    ...
    if mode == "real-ue-direct" and ipsec_mode_value == "native" and msisdn is None:
        raise typer.BadParameter(
...
def packet_command(
    ...
    ipsec_mode: Annotated[
        ...
        typer.Option(
            "--ipsec-mode",
            help="IPsec mode: 'null', 'bypass', or 'native'.",
        ),
    ] = None,
...
def request_command(
    ...
    if mt and mode == "real-ue-direct":
        ...
        preserve_headers = ipsec_mode != "native"
