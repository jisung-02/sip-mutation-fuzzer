...
## 15. Current Runtime Priority

현재 저장소의 기본 운영 기준은 `real-ue-direct`다. Softphone은 로컬 회귀나
비교 실험에 쓸 수 있지만, 문서의 기본 우선순위는 실제 runtime path와
`PACKET_COMPLETENESS` 기준을 따른다.

### 15.1 Real-UE Baseline

| 영역 | 이유 | 기본 타깃 |
|------|------|----------|
| INVITE identity baseline | 현재 real-UE baseline scope의 중심이다 | real-ue-direct |
| INVITE wire/byte mutation | dialog 형성, provisional/final response, IMS private header surface가 크다 | real-ue-direct |
| MT template INVITE | `--mt`가 현재 `3gpp` template을 선택한다 | real-ue-direct |

### 15.2 Runtime-Complete Methods

| 영역 | 이유 |
|------|------|
| MESSAGE, OPTIONS, SUBSCRIBE | stateless runtime path로 직접 실험할 수 있다 |
| ACK, BYE, CANCEL | INVITE transaction/dialog 상태를 만든 뒤 실험한다 |
| INFO, PRACK, REFER, UPDATE | INVITE dialog path 또는 reliable provisional path를 사용한다 |

### 15.3 Generator-Only Methods

| 영역 | 현재 한계 |
|------|----------|
| NOTIFY | generator coverage는 있으나 honest runtime prerequisite state를 소유하지 않는다 |
| PUBLISH | generator coverage는 있으나 runtime handling이 모델링되지 않았다 |
| REGISTER | generator coverage는 있으나 runtime handling이 모델링되지 않았다 |
...
```
1. real-UE INVITE identity baseline
2. MT INVITE baseline with --mt
3. profile-aware wire/byte mutation
4. runtime-complete non-INVITE methods
5. generator-only methods only after honest runtime state exists
```
