...
1. **live resolver (1순위)**: `kamctl ul show`, P-CSCF kamailio 로그의 `PCSCF: REGISTER` 라인, `ip xfrm state` 의 SA selectors. `RealUEDirectResolver.resolve()` 가 이 순서로 시도.
2. **환경변수 (2순위)**: `VMF_MSISDN_TO_IP_<msisdn>=<ip>`. 세션 단위 override.
3. **hardcoded fallback (3순위)**: `src/volte_mutation_fuzzer/sender/real_ue.py:60-63` 의 `_DEFAULT_MSISDN_TO_IP`. live resolver 가 모두 실패할 때만 사용. **stale 값이면 다른 UE 로 패킷이 sneaky 하게 흘러갈 수 있음** — testbed 디바이스 회전 시 이 dict 도 같이 갱신해야 한다.
