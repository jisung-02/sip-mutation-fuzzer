...
| `pixel_ims` | Pixel-class UE 에서 `--pixel` 전달 경로와 함께 쓰는 SDP media / session timer / IMS P-header 집중 계열 |
...
- `pixel_ims` wire 기본 풀은 Pixel/Android IMS 쪽 파싱면을 겨냥해 `pixel_sdp_media_negotiation`, `pixel_session_timer_skew`, `pixel_p_header_pressure`, SDP 구조/byte 변이, Contact alias 변이를 seed 기반으로 고른다. SDP/P-header/timer/alias 전제가 없는 메시지는 안전한 wire 변이로 fallback 한다.
