...
| `iphone_ims` | iPhone/CommCenter IMS 경로에서 SDP media/QoS/AMR-WB, SIP Security Agreement 파라미터, option-tag negotiation, identity/privacy header 를 겨냥한 계열 |
...
- `iphone_ims` wire 기본 풀은 `iphone_sdp_media_negotiation`, `iphone_security_agreement_pressure`, `iphone_option_tag_negotiation`, `iphone_identity_privacy_pressure`, SDP 구조/byte 변이, 안전 wire 변이를 seed 기반으로 고른다. `sec-agree` / option-tag / SDP / identity header 전제가 없는 메시지는 안전한 wire 변이로 fallback 한다.
...
iPhone 16e 슬롯(`222222`)은 먼저 identity baseline 으로 MT-INVITE 응답과 iOS 로그 수집이 모두 정상인지 확인한 뒤 `iphone_ims` 를 적용한다. 운영 가정은 USB iPhone 1대 연결이며, `--ios` 만 켜면 UDID 를 자동 선택해 syslog/crash pull/snapshot 에 동일하게 고정한다. `iphone_ims` 는 기존 testbed 사례의 비-인접 protected port(`port-c=63193`, `port-s=61008`)와 SIP option-tag 사례(`sec-agree`, `precondition`, `100rel`, `timer`)를 별도 변이 표면으로 다룬다. user-less Contact 때문에 resolver 가 실패하는 환경에서만 `VMF_MSISDN_TO_IP_222222=10.20.20.2` 를 지정한다.
